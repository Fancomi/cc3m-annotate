#!/usr/bin/env python3
"""把 adjudicate.html 导出的人工裁决写回 ANALYSIS.md。
...
def main():
    ...
    ap.add_argument("--auto-precision", type=float, default=74.5)
    args = ap.parse_args()
    ...
    L = ["\n## 6. 人工裁决校准（adjudicate.html 结果）\n"]
    L.append(f"对第 5 节的自动判定做人工复核。分层抽样：自动判 YES 的 {NY} 对里抽 {ay[1]} 个，"
    ...
    L.append(f"| 自动判定给出的值 | {args.auto_precision:.1f}% | — | 第 5 节那个下界 |")
    ...
    if delta > 2:
        L.append(f"人工复核后精度比自动判定**高 {delta:.1f} 个百分点** —— 自动判定偏保守，"
                 "确认偏误的影响小于预期，第 5 节的结论可以按下界使用。")
    elif delta < -2:
        L.append(f"人工复核后精度比自动判定**低 {-delta:.1f} 个百分点** —— Qwen 在给自己产出的短语放水，"
                 "第 5 节那个 74.5% **不可作为下界引用**，需改用本节数值。")
    else:
        L.append(f"人工复核与自动判定相差 {abs(delta):.1f} 个百分点（区间内），"
                 "说明该判定器在这个任务上基本可用。")
    L.append("")
    with open(os.path.join(args.dir, "ANALYSIS.md"), "a") as f:
        f.write("\n".join(L) + "\n")
    print(f"a={a*100:.1f}%  b={b*100:.1f}%  真实精度={true_p*100:.1f}% "
          f"[{lo*100:.1f}, {hi*100:.1f}]  判定器准确率={acc*100:.1f}%")
    print("-> 已追加到 ANALYSIS.md 第 6 节")
