#!/usr/bin/env python3
"""生成人工裁决 HTML —— 用于校准"Qwen 判 Qwen"那个 74.5% 的可信度。

统计设计（关键，别改成简单随机抽样）：
  总体 4333 对里 YES 3229 / NO 1104。若直接随机抽，NO 只占 25%，
  样本里 NO 太少，估不准"Qwen 误否"的比例。故改为**分层抽样**：
  从 YES 层和 NO 层各独立抽 n 个，人工判断每个框里短语是否真的存在，得到
      a = P(真存在 | Qwen 判 YES)
      b = P(真存在 | Qwen 判 NO)     ← Qwen 的误否率 = a 的对偶
  再按总体权重还原：
      真实精度 = (3229·a + 1104·b) / 4333
  同时可算出判定器自身的准确率，回答"这个 74.5% 能不能信"。

页面内嵌裁剪图（就是喂给 Qwen 的那张，含 12% padding），人工只需看图判
"短语在不在"，不显示 Qwen 的裁决以免锚定；判完可展开对比。
...
def main():
    ...
    D = args.dir
    rows = [json.loads(l) for l in open(os.path.join(D, "out", "verify_crossfeed.jsonl"))]
    paths = {}
    ...
    for r in samp:
        ...
        if not cb:
            continue
        items.append({"id": r["id"], "phrase": r["phrase"], "verdict": r["verdict"],
...
<div class="note">
<b>这个页面在做什么：</b>之前 ANALYSIS.md 里的"精度 74.5%"是<b>用 Qwen 判 Qwen 自己产出的短语</b>，
存在确认偏误，只能算下界。这里请你对同样的 (短语, 框) 对做人工裁决，用来估出真实精度。<br>
<b>抽样方式是分层的</b>：从 Qwen 判 YES 的 {N_YES} 对里抽 {args.per_stratum} 个，
判 NO 的 {N_NO} 对里也抽 {args.per_stratum} 个 —— 因为 NO 只占总体 25%，
随机抽会导致 NO 样本太少估不准。统计时会按总体权重还原。<br>
<b>判断标准</b>：红框圈出的区域里，能否清楚看到这个短语所指的东西？框大致对上即可，不必像素级精确。
<b>页面刻意不显示 Qwen 的裁决</b>，避免锚定；判完每题可展开对照。<br>
...
function key(it){{return it.id+"|"+it.phrase}}
...
function rec(i,v){{
  ...
  r.className='rev on';
  r.innerHTML=`你判：<span class="v ${{v}}">${{v}}</span> · 判定器(Qwen)判：<span class="v ${{items[i].verdict}}">${{items[i].verdict}}</span>`
...
function render(){{
  ...
  <tr><td>a = P(真存在 | Qwen 判 YES)</td><td><b>${{pct(c.a)}}</b></td><td>n=${{c.ay[1]}}，判定器说有、确实有的比例</td></tr>
  <tr><td>b = P(真存在 | Qwen 判 NO)</td><td><b>${{pct(c.b)}}</b></td><td>n=${{c.an[1]}}，判定器误否的比例</td></tr>
  <tr><td>还原真实精度</td><td><b class="big">${{pct(c.truePrec)}}</b></td>
      <td>(${{DATA.N_YES}}·a + ${{DATA.N_NO}}·b) / ${{DATA.N_YES+DATA.N_NO}}</td></tr>
  <tr><td>自动判定给出的值</td><td>74.5%</td><td>ANALYSIS.md 里那个下界</td></tr>
  <tr><td>判定器自身准确率</td><td><b>${{pct(c.acc)}}</b></td><td>与人工一致的比例（总体加权）</td></tr>
  </table>
  <p style="color:var(--dim);font-size:13px">
  若"还原真实精度" &gt; 74.5%，说明自动判定偏保守（确认偏误没有想象的严重）；
  若明显 &lt; 74.5%，说明 Qwen 在给自己放水，那个下界不可用。<br>
  导出的 JSON 交给 <code>scripts/apply_human_adj.py</code> 可写回 ANALYSIS.md。</p>`;
}}
...
function payload(){{
  return {{N_YES:DATA.N_YES,N_NO:DATA.N_NO,calc:calc(),
    answers:items.map(it=>({{id:it.id,phrase:it.phrase,auto:it.verdict,
...
function pull(){{
  return fetch('/load').then(r=>r.json()).then(j=>{{
    let n=0;
    (j.answers||[]).forEach(x=>{{
      if(x.human){{ ans[x.id+"|"+x.phrase]=x.human; n++; }}
    }});
